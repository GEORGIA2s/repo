...
class Product<T> {
	...
	public String toString() {
		return "Product{" + "data=" + data + ", price=" + price + '}';
	}
}
...
class InventoryManager<T> {
	...
	public void printInventory() {
		...
		for (ProductType type : ProductType.values()) {
			Map<Product<T>, Integer> productMap = inventory.get(type);
			Iterator<Map.Entry<Product<T>,Integer>> it = productMap.entrySet().iterator();
			while(it.hasNext()) {
				Map.Entry<Product<T>, Integer> e = it.next();
				System.out.println("상품 타입: " + e.getKey().getData() + " [재고 : " + e.getValue() + "]\n가격 : "+ e.getKey().getPrice());
			}
...
public class InventoryExample {
	public static void main(String[] args) {
		// 상품 생성
		Product<String> laptop = new Product<>("노트북", 120000.00);
		Product<String> tShirt = new Product<>("티셔츠", 5000.00);
		Product<String> javaBook = new Product<>("자바의 정석", 50000.00);
